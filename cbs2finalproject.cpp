...
{
    int n = V; // Assuming V holds the number of vertices
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
            ...
            if (adj[i][j] == -1)
            {
                adj[i][j] = INT_MAX; // Assuming INT_MAX is used to represent infinity
            }

            // dist from 0 to 0 is 0, 2 to 2 is 0...
            if (i == j)
            {
                adj[i][j] = 0;
            }
    ...
    // Shortest dist from one node-> via then via-> other node
    for (int k = 0; k < n; k++)
    {
        for (int i = 0; i < n; i++)
        {
    ...
    // Revert INT_MAX back to -1 for representing unreachable nodes
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
...
// The model has both user and cabdriver management facility
...
void user::add_new()
{
    ...
    cin >> address;
    // opening a file in the appending and outputing mode
    //  we could have also used the ofstream and ifstream method though
...
void user::book_cab()
{
    ...
    // Call floyd_warshall to update the adjacency matrix with shortest distances
...
// Modify the Details of the cabdriver members
...
void user::cabsearch() // search data of cabdriver
{
    system("cls");
    fstream file;
    int found = 0;
...
// Main Function basically call the base class cabbooking
...
int main()
{
    ...
    cabbooking b;
    // Object of the Base class cabbooking is call here
    b.home_page();
}
